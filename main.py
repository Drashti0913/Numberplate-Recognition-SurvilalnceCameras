...
#APAR Search the letter of state
def extract_num(img_filename):
    ...
    for (x,y,w,h) in nplate:
        ...
        #make the image more darker to identify the LPR
        
        kernel = np.ones((1,1),np.uint8)
        plate = cv2.dilate(img,kernel,iterations=1)
        plate = cv2.erode(img,kernel,iterations=1)
        ...
        cv2.rectangle(img,(x,y-40),(x+w+1,y),(51,51,255),-1)
        cv2.putText(img,read,(x-1,y-10),cv2.FONT_HERSHEY_SIMPLEX,0.9,(255,255,255),2)
